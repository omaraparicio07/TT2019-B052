...
class ParserGDM(object):
    """
    Clase para parsear el gdm de texto simple al modelo orientado a documentos
    """
    def saveModel(self, model):
        ...
        resource = rset.create_resource(URI('venues.xmi'))
        resource.append(model)
        resource.save()
        return

    def getEntity(self, model, name):
        entity = ''
    ...
    def getRefs(self, entity,inclusion):
        refs = []
    ...
    def getRefAliasFromQuery(self, query, aliasEntity):
        refAlias = ''
    ...
    def getAttributeFromAliasEntity(self, query, aliasEntity,attributeReference):
        feature = ''
    ...
    def getCondition(self, query, booleanCondition):
        condition = ''
        aliasEntity = booleanCondition[1].split(".")[0]
        attributeOfAliasEntity = booleanCondition[1].split(".")[1]

        attribute = self.getAttributeFromAliasEntity(query,aliasEntity,attributeOfAliasEntity)
        refAlias = self.getRefAliasFromQuery(query,aliasEntity)
    ...
    def populateEntity(self, model, lines, i):
        line = lines[i]
        name = line.split()[1]
        entity = self.getEntity(model, name)
        count = 1
        ...
        while not "}" in lines[i+count]:
            ...
            if "ref" in ln[0]:
                name = ln[2]
                referencia = self.getEntity(model,ln[1].split("[")[0])
    ...
    def populateQuery(self, model, lines, i):
        start = i
        line = lines[i]
        name = line.split()[1].strip(":")
        query = self.getQuery(model, name)
        ...
        while not "from " in lines[i+count]:
            count += 1
        ln = lines[i+count].split()
        entity = self.getEntity(model,ln[1])
        ...
        for inclusion in inclusions:
            ...
            refs = self.getRefs(entity,inclusion)
            refAlias = query.from_.alias
        ...
        for select in selects:
            if " as " in select:
                ...
                attribute = self.getRefs(entity,select.split())
        ...
        if len(conditions) == 1:
            left = self.getCondition(query, conditions[0])
            condition = gdm.AndConjunction(left=left)
        else:
            left = self.getCondition(query, conditions[0])
            right = self.getCondition(query, conditions[1])
    ...
    def main(self, gdmFileName=''):
        ...
        for i in range(len(lines)):
            ...
            # Si es una entidad
            if "entity" in line:
                self.populateEntity(model, lines, i)
        ...
        for i in range(len(lines)):
            ...
            # Si es una consulta
            if "query" in line:
                self.populateQuery(model,lines,i)
